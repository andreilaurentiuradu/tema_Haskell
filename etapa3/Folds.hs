...
instance Show TransformationAST where
    ...
    -- verifici ce tip e
        ...
        | Combine ts <- transformation        = "[" ++ intercalate "," (map show ts) ++ "]"
...
instance Show RegionAST where
    show = (`showHelper` 0)
      where
        showHelper = foldRegionAST combiner

        combiner :: RegionCombiner (Int -> String)
        combiner (FromPoints points) = \level -> indent level ++ "FromPoints " ++ show points
        combiner (Rectangle w h) = \level -> indent level ++ "Rectangle " ++ show w ++ " " ++ show h
        combiner (Circle r) = \level -> indent level ++ "Circle " ++ show r
        combiner (Complement r) = \level -> indent level ++ "~\n" ++ r (level + 1)
        combiner (Union r1 r2) = \level -> indent level ++ "+\n" ++ r1 (level + 1) ++ "\n" ++ r2 (level + 1)
        combiner (Intersection r1 r2) = \level -> indent level ++ "*\n" ++ r1 (level + 1) ++ "\n" ++ r2 (level + 1)
        combiner (Transform t r) = \level -> indent level ++ show t ++ "\n" ++ r (level + 1)

        indent :: Int -> String -- pentru nr de spatii
...
instance Num RegionAST where
    ...
    region1 - region2 = intersection region1 (complement region2)
